...
def execute_R_scripts():
    ...
    subprocess.call("/usr/bin/Rscript --vanilla main.R", shell=True) 

def create_markdown():
    """ Create markdown file which Kevin's figures are embedded. """
    mdFile = MdUtils(file_name='report')
    mdFile.new_header(
    ...
    mdFile.new_paragraph(Html.image(path="images/fig1.png", size='600', align='center'))
    mdFile.new_paragraph('**Figure 2.** Association between non-residential mobility and COVID-19 case growth across 7 Canadian provinces, March 15, 2020 to January 16, 2021.')
    mdFile.new_paragraph(Html.image(path="images/fig2.png", size='900', align='center'))
    ...
    mdFile.new_paragraph(Html.image(path="images/fig3.png", size='900', align='center'))
...
def main():
    ...
    create_markdown() # Python function to generate the markdown where Kevin's figures will be embedded 
    push_to_github()
